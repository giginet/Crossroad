...
public typealias SimpleRouter = LegacyRouter<Void>

public final class LegacyRouter<UserInfo> {
    private enum Prefix {
        case scheme(String)
        case url(URL)
    }
    private let prefix: Prefix
    private var routes: [LegacyRoute<UserInfo>] = []
    ...
    internal func register(_ route: LegacyRoute<UserInfo>) {
    ...
    public func register(_ routes: [(String, LegacyRoute<UserInfo>.Handler)]) {
        for (pattern, handler) in routes {
            ...
            let route = LegacyRoute(pattern: patternURL, handler: handler)
            register(route)
        }
    }
}

public extension LegacyRouter where UserInfo == Void {
    @discardableResult
